...
func (r *MyKindReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if apierrors.IsNotFound(err) {
		...
		deployment = *buildDeploymentPodJob(myKind)
...
func buildDeploymentPodJob(myKind mygroupv1beta1.MyKind) *apps.Deployment {
	deployment := apps.Deployment{
...
func (r *MyKindReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(&core.Pod{}, deploymentOwnerKey, func(rawObj runtime.Object) []string {
		pod := rawObj.(*core.Pod)
		podInfo := metav1.GetControllerOf(pod)
		if podInfo.APIVersion != "v1" || podInfo.Kind != "MyKind"{
			return []string{"invalid API Version or Kind"}
		}

		return []string{podInfo.APIVersion,"controller is valid"}
	}); err != nil {
		return err
	}
	...
	checkNamespace := func(e event.CreateEvent) bool {
		if e.Meta.GetNamespace() != "vishu" {
			log.Fatal("resources not running in vishu namespace")
			return false
		}
		return true
	}
